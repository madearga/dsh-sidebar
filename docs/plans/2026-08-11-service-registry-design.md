...
将 `dsh-sidebar` 从一个"纯消费方"插件改造为"服务提供者"：

1. 允许其他 DSH 插件向 dsh-sidebar **注册新的侧边栏页面（tab 类型）**
2. 允许其他 DSH 插件向 dsh-sidebar **注册文件类型预览器**
...
- **双 cordis 实例**：外部插件解析不到 DSH monorepo 的 cordis augmentation，必须由 dsh-sidebar 自己导出 `declare module 'cordis'` augmentation
...
| 方案 | 机制 | 评估 |
|---|---|---|
| **A. `ctx.provide` + 服务对象（采纳）** | client `apply()` 内 `ctx.provide('dshSidebar', service)`，service 暴露 `registerTab/registerFileViewer` 返回 disposer | registry 语义自然；disposer 生命周期贴合 cordis fiber；precedent：`dsh/packages/client/locale/src/client/index.ts:362` |
| B. `ctx.slots.inject` 声明式 slot | 声明 `dshSidebar.tabType` 等 slot | 偏 UI 链替换，对带元数据 + disposer 的 registry 别扭 |
...
通过 `src/index.ts` 的 `export type * from './types.ts'` re-export 到主入口。Consumer 插件 `import type {} from 'dsh-sidebar'` 触发合并。
...
export const inject = ['dshSidebar', 'slots']
...
| 风险 | 缓解 |
|---|---|
| 双 cordis 实例导致 consumer 拿不到 `ctx.dshSidebar` 类型 | 由 dsh-sidebar 自己 `declare module 'cordis'`，consumer `import type {}` 触发合并 |
| 注册顺序：consumer 在 dsh-sidebar 之前激活导致 inject 等待 | cordis `inject` 自带就绪等待，consumer 不会在服务就绪前激活 |
...
4. `src/client/index.tsx` 改造：`ctx.provide('dshSidebar', service)` + `registerBuiltins(service)`
