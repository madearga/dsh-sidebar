...
> 日期:2026-08-12
> 状态:已实现
> 范围:`dsh-sidebar` 插件,客户端 bundle 拆分
...
- `ctx.dshSidebar` 服务方法签名与行为零变化;懒加载只作用于**内置** descriptor,外部插件注册的原样存储/渲染。
...
- chunk 注册 id 恒定 `dsh-sidebar/<name>`,两渠道共用(每页只活一个渠道——双渠道并存本就双挂载,属既有破损组合);
