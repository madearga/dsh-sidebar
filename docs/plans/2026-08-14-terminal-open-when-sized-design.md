...
修复 [issue #25](https://github.com/madearga/DSH-sidebar/issues/25)：WKWebView 下点击「展开底部面板」后底部面板内容空白、无终端，控制台报 `Cannot read properties of undefined (reading 'dimensions')` 与 `(reading 'handleResize')`。
...
const cancelOpen = openWhenSized(host, () => {
  try {
    ...
    console.error('[dsh-sidebar] xterm open failed:', error)
  }
})
```
