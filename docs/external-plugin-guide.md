# 外部插件接入指南：基于 dsh-sidebar 实现新页面

> 面向 **消费插件开发者**：如何让你的插件向 dsh-sidebar 注册新的侧边栏页面（tab）和文件类型预览器。
>
> 适用版本：**v0.4.0+**（`ctx.dshSidebar` 服务）；声明式设置 **v0.4.1+**；text/number 设置行 **v0.11.0+**；本文档其余新 API（badge/生命周期/定向打开/插件设置/版本探测等）**v0.12.0+**。当前版本 v0.12.0。
...
dsh-sidebar 从 v0.4.0 起把自己改造成一个**注册表服务**：
...
关键机制一句话：dsh-sidebar 的 client half 在 `apply()` 开头执行 `ctx.provide('dshSidebar', service)`（`src/client/index.tsx`），消费插件在 `inject` 里声明 `'dshSidebar'`，Cordis 保证服务就绪后才激活你的插件，然后你调用 `ctx.dshSidebar.registerTab(...)` / `registerFileViewer(...)` 完成注册，返回的 disposer 由 Cordis fiber 在卸载（HMR / 禁用）时自动调用。

> ⚠️ **服务只在 client half**：`ctx.dshSidebar` 只存在于浏览器侧。你的插件 **host 半没有这个服务**；host 半需要读 dsh-sidebar 状态时，走它自己的 HTTP/WS 路由（`/sidebar/api/*`、`/sidebar/file`、`/sidebar/ws/*`），不走服务。
...
外部插件在 DSH monorepo 之外解析，拿不到官方 cordis 的 augmentation，因此 `ctx.dshSidebar` 不会自动出现在你的 `Context` 类型上。解法由 dsh-sidebar 自己提供：

```ts
import type {} from 'dsh-sidebar'  // 触发 declare module 'cordis' 类型合并
```
...
```jsonc
{
  ...
  "peerDependencies": {
    "cordis": "^4.0.0-rc.7",
    "dsh-sidebar": "workspace:*"
  },
  "peerDependenciesMeta": {
    "dsh-sidebar": { "optional": true }
  }
}
```

- `dsh-sidebar` 必须是 **peerDependency**（不是 dependency），避免两份实例；
- `optional: true`：dsh-sidebar 未安装时你的插件照常加载，注册代码因为 `ctx.dshSidebar` 为 undefined 而安全跳过。
...
// 方式二：子路径（与主入口等价）
import type { TabDescriptor } from 'dsh-sidebar/client/service'  // 别名 ./client/api
```
...
> 💡 **类型合并触发路径**：`import type {} from 'dsh-sidebar/client/service'` 同样会加载 `Context` 的 augmentation（`declare module 'cordis'` 在 context-types.d.ts 中）——**纯浏览器侧插件建议走 `client/service` 路径**，避免拉进宿主半的 Node 类型图（主入口 `dsh-sidebar` 的声明面含宿主代码；宿主消费者本就处于 Node 环境则无所谓）。client 可达声明图（`client/*` + context-types + html-route + prefs-shared）自 v0.12.0 起**零 Node 依赖**（`scripts/check-consumer-types.sh` 守护），没有 `@types/node`、`skipLibCheck: false` 也能编译。
...
import type {} from 'dsh-sidebar'          // 触发 ctx.dshSidebar 类型合并
import type { Context } from 'cordis'

export const inject = ['dshSidebar', 'slots']   // 声明服务依赖（slots 可选，按需）

export function apply(ctx: Context): void {
  // 注册一个 sidebar tab：ctx.effect 包裹 → 卸载时自动撤销注册（HMR-safe）
  ctx.effect(() =>
    ctx.dshSidebar.registerTab({
      id: 'my-plugin:db',
      title: () => 'Database',
      icon: <DbIcon />,
      order: 50,
  ...
  // 注册一个文件预览器
  ctx.effect(() =>
    ctx.dshSidebar.registerFileViewer({
      id: 'my-plugin:csv',
      exts: ['csv'],
...
- `inject = ['dshSidebar']` 让 Cordis 在 dsh-sidebar 激活后才激活你的插件，注册时机无忧（顺序无关）。
- 注册在 `apply` 内任意时刻都行；服务在 dsh-sidebar 的 `apply()` 开头就绪。
...
interface TabComponentProps {
  ...
  store: SidebarStore          // dsh-sidebar 的状态 store（可调 reduce 等）
...
ctx.dshSidebar.openTab({ type: 'my-plugin:doc', title: 'Spec.md', id: 'doc:spec' })
```
...
你的 tab / viewer 组件运行在浏览器里，与内置视图同源同权。访问文件/会话数据直接 `fetch` dsh-sidebar 的 JSON API（内置 `src/client/api.ts` 的封装就是干这个的，你可以在自己插件里复制这个 fetch 模式）：
...
> 注：内置的 `api.ts` 是 dsh-sidebar 内部模块，外部插件 **不要** value-import 它（构建纯度门会挡）；按上表模式自己 fetch 即可。所有路由带与 `/api` 相同的 Host 头信任围栏，浏览器同源访问天然通过。
...
```ts
if (ctx.dshSidebar.features.includes('badge')) {
  // 使用 TabDescriptor.badge
}
if (ctx.dshSidebar.version >= '0.12.0') { /* 字符串比较即可：minor 只增 */ }
```
...
- **注册时机**：dsh-sidebar 在 `apply()` 开头 `ctx.provide('dshSidebar', service)`，你的 `inject = ['dshSidebar']` 保证你激活时服务已就绪；注册顺序无关紧要。
...
| 陷阱 | 说明 |
|---|---|
| **构建纯度门** | client bundle 禁止 value-import `@dsh-external/*` 或非白名单的 `@deepseek-ai/*`（`tsdown.config.ts` 的 `dsh-client-bundle-purity` 插件）；**类型 `import type {}` 会被擦除，不触发门禁**——类型可自由共享，运行时符号不行。所有跨插件交互走 `ctx.dshSidebar` 方法调用 |
| **双 cordis 实例** | 外部插件解析不到 DSH monorepo 的 cordis augmentation；dsh-sidebar 自己重述了 `interface Context { dshSidebar: ... }`（`src/context-types.ts`），你 `import type {} from 'dsh-sidebar'` 即拿到类型 |
| **ModuleLoader 不跨插件** | 运行时 `require()` 虽支持跨 bundle，但被构建门挡；所有交互走 `ctx.dshSidebar` 方法调用 |
| **host 半无此服务** | `ctx.dshSidebar` 只在 client 侧存在；host 半需要 dsh-sidebar 数据走 `/sidebar/api/*` HTTP 路由 |
| **portal 限制** | 整面板 slot 由 ui-layout 独占，外部 tab 只能进入 dsh-sidebar 的 portal 内部，无法全屏替换整个面板 |
| **id 冲突** | `registerTab` / `registerFileViewer` 对重复 id 抛错；建议用包前缀（`my-plugin:xxx`） |
| **不要 value-import `dsh-sidebar`** | 即使是 `./client/service` 子路径，运行时落点也是 client bundle；只做 type-only import |
| **client 声明图零 Node 依赖（v0.12.0+）** | `dsh-sidebar/client/service` 的可达声明面（含 `Context`）不引用 `node:*` / `Buffer`——纯浏览器侧插件无需 `@types/node`，`skipLibCheck: false` 也能编译（`scripts/check-consumer-types.sh` 守护；主入口含宿主声明，宿主消费者本就处于 Node 环境） |
...
export const inject = ['dshSidebar']
...
dsh-sidebar 的内置 tab 和 viewer 就是参考实现（"吃狗粮"）：
...
第一个通过 `ctx.dshSidebar` 接入的三方插件：[dsh-sentinel](https://github.com/fuhefei/dsh-sentinel) —— 条件驱动的 agent 唤醒系统（文件/进程/端口/HTTP/命令/webhook 传感器，条件达成自动唤醒休眠会话）。

- **接入方式**：可选软依赖——client half 本地重述最小服务契约（`registerTab`），未安装 dsh-sidebar 时注册静默跳过，插件原有表面不受影响；
- **注册内容**：`dsh-sentinel:watches` tab（order 60，单实例）：全服务器监控表 + 最近触发历史；
- **类型处理**：未 value-import `dsh-sidebar`，构建零耦合；与 §2 的 `import type {}` 方案可互换；
