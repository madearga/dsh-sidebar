...
| 🐛 Fixes | Remote access 403 (trust fence now uses `trustedHosts`), sidebar crash [#31](https://github.com/madearga/dsh-sidebar/issues/31), Windows HTML-preview drive-path | |
...
- **🔌 Service API**: exposes `ctx.dshSidebar` — other plugins can register tabs and file viewers (the 7 built-in tabs + 9 viewers share the same service, see [AGENTS.md](./AGENTS.md) and the [external plugin guide](./docs/external-plugin-guide.md))
...
```sh
curl -fsSL https://raw.githubusercontent.com/madearga/dsh-sidebar/main/scripts/install.sh | bash
```

**Windows (PowerShell 5.1+ / pwsh)**:

```powershell
irm https://raw.githubusercontent.com/madearga/dsh-sidebar/main/scripts/install.ps1 | iex
```
...
```sh
# macOS / Linux
curl -fsSL https://raw.githubusercontent.com/madearga/dsh-sidebar/main/scripts/install.sh | bash -s 0.12.1 --restart

# Windows PowerShell
& ([scriptblock]::Create((irm 'https://raw.githubusercontent.com/madearga/dsh-sidebar/main/scripts/install.ps1'))) -Version 0.12.1 -Restart
```
...
# ③ Install and auto-mount (no @version = npm's latest; pin with dsh-sidebar@0.12.1)
...
# ② Allow fresh releases (once; if the key already exists, merge - dsh-sidebar under it instead)
Add-Content -Path pnpm-workspace.yaml -Value "`nminimumReleaseAgeExclude:`n  - dsh-sidebar"
...
3. Runs `dsh plugin --profile web add dsh-sidebar`: registers the dependency → detects `dsh.bundle.patch` → auto-appends the plugin to `dsh.profile.bundles`;
4. Removes any leftover hand-written mount line to avoid double-mounting (two sidebars on the page).

`curl | bash` / `irm | iex` executes remote code — the scripts are open source in the repo (`scripts/install.sh` / `scripts/install.ps1`); download and review them first if you prefer. The plugin ships as npm package `dsh-sidebar@0.12.1` and mounts via `dsh.bundle.patch` (the shipped `cordis.patch.yml`), so the DSH source is never modified.
...
| Two sidebars on the page | Double-mount: `~/.dsh/profiles/web/cordis.patch.yml` still has the old hand-written `- insert: ... dsh-sidebar ...` line — delete it (the one-click script cleans it). |
...
```text
1. git clone https://github.com/madearga/dsh-sidebar.git ~/Code/DSH-sidebar
   cd ~/Code/DSH-sidebar && pnpm install && pnpm build
2. In ~/.dsh/profiles/web/package.json dependencies write "dsh-sidebar": "link:<absolute path of the clone>"
...
Update: `git pull && pnpm install && pnpm build` → just hard-refresh the browser (client changes hot-reload; only host-half changes need a DSH restart). To switch back to the npm channel, restore `"dsh-sidebar": "^0.12.1"` and re-run `pnpm install`.
...
```sh
git clone https://github.com/madearga/dsh-sidebar.git && cd DSH-sidebar
...
dsh registry enable madearga/dsh-sidebar
```
...
Since v0.4.0 the plugin exposes the `ctx.dshSidebar` service — other plugins can register sidebar pages and file viewers (the 7 built-in tabs + 9 viewers go through the same service, eating our own dog food):

```ts
import type {} from 'dsh-sidebar'  // triggers the ctx.dshSidebar type merge
export const inject = ['dshSidebar']
export function apply(ctx: Context) {
  ctx.effect(() => ctx.dshSidebar.registerTab({
...
The dashed cards at the end of the "Sidebar content" / "File viewers" grids in the "Side Cards" settings section open the **Add tab plugins** / **Add preview plugins** modals: each declares its open extension point, offers a "**Browse more plugins on GitHub**" button (the [GitHub topic `dsh-sidebar`](https://github.com/topics/dsh-sidebar)), and lists the recommended catalog (name / repo / description / install script) — "**Open**" jumps to the repo, "**Copy**" writes the install command to the clipboard.

**Curating a new plugin**: append a `PluginEntry` to [`src/client/plugins-tabs.ts`](./src/client/plugins-tabs.ts) (tab registrations) or [`src/client/plugins-viewers.ts`](./src/client/plugins-viewers.ts) (file-previewer registrations) and tag your repo with the `dsh-sidebar` topic; data integrity is guarded by `tests/plugin-list.spec.ts`.
