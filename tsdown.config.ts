/**
 * tsdown build for dsh-sidebar: the host-half lib (lib/index.js and
 ...
 *   the package-name id `dsh-sidebar` (the client-modules compose
 ...
 *   `madearga/dsh-sidebar` (the registry browser-side `arrive()`
 ...
 * differing: the official channel uses the package name (`dsh-sidebar`)
 * and the registry channel uses the manifest id
 * (`madearga/dsh-sidebar`).
 ...
 *
 * Chunk css tags use the constant plugin id `dsh-sidebar` (matching
...
function chunkBundle(name: string): UserConfig {
  return {
    ...
    noExternal: (id: string) => (CLIENT_EXTERNALS.includes(id) ? undefined : true),
    plugins: [purityGatePlugin(), makeCssPlugin('dsh-sidebar')],
    outputOptions: {
...
export default [
  ...
  clientBundle('dsh-sidebar', 'client.js'),
  // Plugin-registry channel: bundle id = manifest id (dsh.plugin.json `id`).
  clientBundle('madearga/dsh-sidebar', 'client-registry.js'),
