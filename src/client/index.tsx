/**
 * Client half of dsh-sidebar: resolves the user's "Side card"
...
export function apply(ctx: Context): void {
  ...
  ctx.effect(
    ...
    'dsh-sidebar: register built-in tabs and viewers',
  )
  ...
  const fail = (phase: string, error: unknown): void => {
    console.error(`[dsh-sidebar] ${phase} error:`, error)
    try {
      ...
      bar.textContent = `[dsh-sidebar] ${phase} error: ${error instanceof Error ? error.message : String(error)}`
  ...
  try {
    ...
    ctx.effect(() => {
      ...
      void (async () => {
        ...
        try {
          ...
          host.setAttribute('data-dsh-sidebar', '')
    ...
    ctx.effect(
      ...
      'dsh-sidebar: turn-tail interception',
    )
    ...
    ctx.effect(
      ...
      'dsh-sidebar: open-path interception',
    )
        ...
        try {
          ...
          return registerLinkInterception({
            ...
            openInSidebar: (url) => {
              ...
              ctx.dshSidebar?.openTab({ type: 'browser', url, title })
            },
    ...
    ctx.effect(
      ...
      'dsh-sidebar: IME composition guard',
    )
