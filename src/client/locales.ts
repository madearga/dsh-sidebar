...
export const zh = {
  ...
  addPluginsTabDesc: '侧边栏页面（Tab）可以由插件扩展。插件通过 ctx.dshSidebar 服务注册；点击「安装」复制安装命令，粘贴到 DSH 所在环境的终端执行。',
  addPluginsViewerDesc: '文件预览器可以由插件扩展。插件通过 ctx.dshSidebar 服务注册；点击「安装」复制安装命令，粘贴到 DSH 所在环境的终端执行。',
  addPluginsBrowseMore: '在 GitHub 上浏览更多插件（topic: dsh-sidebar）',
  addPluginsRecommended: '推荐插件',
  addPluginsEmpty: '暂未收录插件，欢迎在 GitHub topic 下发布你的插件',
  openPlugin: '跳转',
  copyInstall: '复制安装命令',
  pluginOfficeDesc: '为 dsh-sidebar 编辑器提供 Office 三件套预览（.docx / .xlsx / .pptx），把重型 Office 渲染库拆出主包、按需安装',
...
export const en: Record<keyof typeof zh, string> = {
  ...
  addPluginsTabDesc: 'Sidebar pages (tabs) can be extended by plugins. Plugins register through the ctx.dshSidebar service; clicking Install copies the install command — paste it into a terminal where your DSH profile lives and run it.',
  addPluginsViewerDesc: 'File previewers can be extended by plugins. Plugins register through the ctx.dshSidebar service; clicking Install copies the install command — paste it into a terminal where your DSH profile lives and run it.',
  addPluginsBrowseMore: 'Browse more plugins on GitHub (topic: dsh-sidebar)',
  ...
  pluginOfficeDesc: 'Office-suite preview (.docx / .xlsx / .pptx) for the dsh-sidebar editor, keeping the heavy Office render libraries out of the core bundle',
 ...
 */
export const LOCALE_NS = 'dshSidebar'
