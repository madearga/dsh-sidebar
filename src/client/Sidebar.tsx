...
function TabContent(props: {
  ...
  const descriptor = ctx.dshSidebar?.getTab(tab.type)
...
export function Sidebar(props: { ctx: Context; store: SidebarStore }) {
    ...
    const connect = (): void => {
      ...
      socket.onmessage = (event) => {
        ...
        try {
          ...
          store.reduce(s => ctx.dshSidebar?.isTabEnabled('terminal') === false
            ? s
      ...
      socket.onclose = () => {
        ...
        if (failures >= FAILURE_LIMIT) {
          console.error('[dsh-sidebar] agent-terminals connection failed; stopping reconnect loop', sessionId)
          return
        }
  ...
  useEffect(() => {
    ...
    if (ctx.dshSidebar?.isTabEnabled('subagent') === false) return
    ...
    ctx.dshSidebar?.openTab({ type: 'subagent', title: t('subagent') })
  ...
  useEffect(() => {
    ...
    if (ctx.dshSidebar?.isTabEnabled('terminal') === false) return
    ...
    ctx.dshSidebar?.openTab({ type: 'terminal' })
  ...
  const actions: WorkbenchActions = useMemo(() => ({
    closeTab: (paneId, tabId) => {
      ...
      ctx.dshSidebar?.closeTab(tabId, sessionId === undefined ? undefined : { sessionId, cwd })
    ...
    activateTab: (paneId, tabId) => {
      ...
      ctx.dshSidebar?.activateTab(tabId, sessionId === undefined ? undefined : { sessionId, cwd })
    },
  ...
  const tabBadgeOf = (tab: SidebarTab): ReactNode => {
    ...
    try {
      ...
      console.error('[dsh-sidebar] tab badge error:', error)
      return null
    }
