...
function safeCall(fn: () => void): void {
  try {
    ...
    console.error('[dsh-sidebar] plugin callback error:', error)
  }
}
...
export function createSidebarService(store: SidebarStore): SidebarService {
  ...
  const registerTab = (descriptor: TabDescriptor): (() => void) => {
    if (tabs.has(descriptor.id)) {
      throw new Error(`[dsh-sidebar] tab type "${descriptor.id}" already registered`)
    }
  ...
  const registerFileViewer = (descriptor: FileViewerDescriptor): (() => void) => {
    if (viewers.has(descriptor.id)) {
      throw new Error(`[dsh-sidebar] file viewer "${descriptor.id}" already registered`)
    }
  ...
  const openTab = (seed: OpenTabSeed, scope?: SessionScope): void => {
    ...
    if (!isTabEnabled(seed.type)) {
      console.warn(`[dsh-sidebar] tab type "${seed.type}" is disabled in the side card settings`)
      return
    }
