...
# imports `dsh-sidebar/client/service` WITHOUT @types/node and with
...
ln -s "$(pwd)" "$WORK/node_modules/dsh-sidebar"
...
ctx.dshSidebar.registerTab(tab)
...
ctx.dshSidebar.registerFileViewer(viewer)
const seed: OpenTabSeed = { type: 'x:tab', title: 'X', meta: { a: 1 } }
ctx.dshSidebar.openTab(seed, { sessionId: 's1' })
ctx.dshSidebar.openFile({ sessionId: 's1' }, '/p/a.csv')
ctx.dshSidebar.updateTab('t', { meta: { b: 2 } })
const snap: SidebarSnapshot = ctx.dshSidebar.getSnapshot()
const prefs: SidebarPrefs = snap.prefs
const store: SidebarStore = null as unknown as SidebarStore
void prefs; void store; void ctx.dshSidebar.version; void ctx.dshSidebar.features
...
if grep -E "dsh-sidebar|lib/types|check\.ts" "$WORK/strict.log" | grep -v "node_modules/cordis\|node_modules/cosmokit"; then
  echo "[check-consumer-types] FAIL: errors in the dsh-sidebar declaration surface:" >&2
  grep -E "dsh-sidebar|lib/types|check\.ts" "$WORK/strict.log" >&2
  exit 1
fi
if [ "$STATUS" -ne 0 ]; then
  echo "[check-consumer-types] pass 2 note: strict mode only reports upstream declaration noise:"
  grep -v "dsh-sidebar\|lib/types\|check\.ts" "$WORK/strict.log" | head -5
fi
