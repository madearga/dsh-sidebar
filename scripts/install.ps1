# =============================================================================
# dsh-sidebar 一键安装脚本（官方 CLI 方式，Windows PowerShell 5.1+ / pwsh）
#
# 通过 DSH 官方插件命令安装 npm 包并自动挂载：
#   dsh plugin --profile web add dsh-sidebar@<version>
#
...
#   # 指定版本 / 装完重启
#   & ([scriptblock]::Create((irm 'https://raw.githubusercontent.com/madearga/dsh-sidebar/main/scripts/install.ps1'))) -Version 0.10.2 -Restart
#   # 本地保存后运行
...
if ($DryRun) {
  ...
  Say "[dry-run] 步骤 4：幂等移除 $PATCH_YML 里旧的 dsh-sidebar 手动挂载行（避免双挂载）"
...
if ($bundles -notcontains $PKG) {
  Warn 'dsh-sidebar 未出现在 dsh.profile.bundles 中——挂载未注册。'
...
while (i < lines.length) {
  ...
  if (/^[ \t]*- insert:\s*$/.test(line)) {
    ...
    if (block.some((l) => /id:\s*dsh-sidebar\b/.test(l))) {
...
if ($mountResult -eq 'removed') {
  Say "已从 $PATCH_YML 移除旧的 dsh-sidebar 手动挂载行（bundle 通道接管挂载）"
} else {
  Say '无旧手动挂载行，跳过'
}
